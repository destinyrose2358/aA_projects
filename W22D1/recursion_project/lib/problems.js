...
function fileFinder(directories, targetFile) {
    for (let key in directories) {
        if (targetFile === key) return true;
        if ((typeof directories[key] === "object") && fileFinder(directories[key], targetFile)) return true; 
    }
    return false;
}
...
function pathFinder(directories, targetFile) {
    for (let key in directories) {
        if (targetFile === key) return `/${targetFile}`;
        if (typeof directories[key] === "object") {
            let possiblePath = pathFinder(directories[key], targetFile);
            if (typeof possiblePath === "string") return key + possiblePath;
        }
    }
    return null;
}
